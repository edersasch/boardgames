...
MUEHLE_LIB_EXPORT std::int32_t board_offset(const muehle::Muehle_Key& key);
MUEHLE_LIB_EXPORT Muehle_Key constellation_to_key(const Muehle_Constellation& constellation, const bool whites_turn);
MUEHLE_LIB_EXPORT std::int32_t game_phase(const Muehle_Key& key);
MUEHLE_LIB_EXPORT const std::vector<std::int32_t>& free_fields(const Muehle_Key& key);
MUEHLE_LIB_EXPORT const std::vector<std::int32_t>& fields_of_selectable_pieces(const Muehle_Key& key);
MUEHLE_LIB_EXPORT const std::vector<std::int32_t>& occupiable_fields(const Muehle_Key& key, const std::int32_t startfield_of_piece);
MUEHLE_LIB_EXPORT bool closed_muehle(const Muehle_Key& key, const std::int32_t field);
MUEHLE_LIB_EXPORT std::int64_t prisoner_count(const Muehle_Key& key);
MUEHLE_LIB_EXPORT std::pair<std::vector<std::int32_t>, Muehle_Key> occupy(Muehle_Key key, const std::int32_t from, const std::int32_t to);
MUEHLE_LIB_EXPORT bool can_slide(const Muehle_Key& key, const std::size_t field);
MUEHLE_LIB_EXPORT const std::vector<std::int32_t>& free_adjacent_fields(const Muehle_Key& key, const std::size_t field);
MUEHLE_LIB_EXPORT std::string diff_text(const boardgame::Field_Number_Diff& fndiff);
MUEHLE_LIB_EXPORT Muehle_Key switch_player(const Muehle_Key& key);
MUEHLE_LIB_EXPORT bool is_boring_move(const muehle::Muehle_Key& key, const muehle::Muehle_Key& successor);
...
struct MUEHLE_LIB_EXPORT Engine_Helper {
