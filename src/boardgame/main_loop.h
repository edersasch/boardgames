...
public:
    ...
    friend void engine_future(const Main_Loop& ml, std::future<void>&& efu) { ml.main_loop_concept->ef(std::move(efu)); }
...
private:
    struct Main_Loop_Concept
    {
        ...
        virtual void ef(std::future<void>&& efu) const = 0;
    ...
    struct Main_Loop_Model : Main_Loop_Concept
    {
        ...
        void ef(std::future<void>&& efu) const override { engine_future(ml, std::move(efu)); }
