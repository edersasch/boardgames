    ...
    : move_list_component(v.engine())
{
    ...
    mlq.reset(new boardgame_qml::Multi_Move_List_Qml(v.engine(), {move_list_root_entry1, move_list_root_entry2}, "test", "tst"));
    ...
    connect(mlq.get(), &boardgame_qml::Multi_Move_List_Qml::request_set_current_move_and_branch_start_id, &move_list_slots, &Multi_Move_List_Qml_Test_Slots::request_set_current_move_and_branch_start_id);
    auto set_current_move = [this](const int move_id) { current_move(mlq.get(), move_id); };
    ON_CALL(move_list_slots, request_set_current_move_and_branch_start_id(_)).WillByDefault(Invoke(set_current_move));
    connect(mlq.get(), &boardgame_qml::Multi_Move_List_Qml::request_move_list_forward, &move_list_slots, &Multi_Move_List_Qml_Test_Slots::request_move_list_forward);
    connect(mlq.get(), &boardgame_qml::Multi_Move_List_Qml::request_move_list_back, &move_list_slots, &Multi_Move_List_Qml_Test_Slots::request_move_list_back);
    connect(mlq.get(), &boardgame_qml::Multi_Move_List_Qml::added_move, &move_list_slots, &Multi_Move_List_Qml_Test_Slots::added_move);
    connect(mlq.get(), &boardgame_qml::Multi_Move_List_Qml::request_move_list_import, &move_list_slots, &Multi_Move_List_Qml_Test_Slots::request_move_list_import);
    connect(mlq.get(), &boardgame_qml::Multi_Move_List_Qml::request_move_list_export, &move_list_slots, &Multi_Move_List_Qml_Test_Slots::request_move_list_export);
    connect(mlq.get(), &boardgame_qml::Multi_Move_List_Qml::request_delete_branch, &move_list_slots, &Multi_Move_List_Qml_Test_Slots::request_delete_branch);
    ...
    initial_constellation(mlq.get(), 0);
    processEvents();
}
...
void Multi_Move_List_Qml_Test::add_sequence(int move_id, int number_of_moves)
{
    ...
    for (; move_id < end; move_id += 1) {
        ...
        add_move(mlq.get(), move_id, std::to_string(move_id), {move_id});
    }
}
...
TEST_F(Multi_Move_List_Qml_Test, controls)
{
    ...
    current_move(mlq.get(), 0);
    EXPECT_CALL(move_list_slots, added_move(2, 5));
    add_move(mlq.get(), 2, "02", {5});
    processEvents();
...
TEST_F(Multi_Move_List_Qml_Test, delete_and_cut_off)
{
    ...
    current_move(mlq.get(), 50);
    add_sequence(101, 100);
    current_move(mlq.get(), 150);
    add_sequence(201, 100);
    current_move(mlq.get(), 250);
    add_sequence(301, 100);
    ...
    */
    ...
    delete_move(mlq.get(), 200);
    delete_move(mlq.get(), 199);
    delete_move(mlq.get(), 198);
    ...
    cut_off(mlq.get(), 151); // 201 - 250 move behind 150
    ...
    cut_off(mlq.get(), 301); // 251 - 300 move behind 250, which is behind 101 now
    ...
    cut_off(mlq.get(), 51); // 101 - 300 move behind 50
