...
Move_List_Qml::Move_List_Qml(QQmlEngine* engine, QQuickItem* move_list_root_entry)
    ...
    , move_button(engine, QUrl(QStringLiteral("qrc:/qt/qml/src/boardgame_qml/Move_Button.qml")))
    , move_list_entry(engine, QUrl(QStringLiteral("qrc:/qt/qml/src/boardgame_qml/Move_List_Entry.qml")))
{
...
std::vector<QQuickItem*> Move_List_Qml::add_move(const int move_id, const std::string& description)
{
    ...
    auto reparent_buttons = [this] (const auto& button_container, QQuickItem* dest) {
        for (auto rp_button : button_container) {
            auto rp_buttonid = QQmlProperty(rp_button, "move_id").read().toInt();
    ...
    for (const auto& button : items) {
        if (auto buttonid = QQmlProperty(button, "move_id").read().toInt(); buttonid > current_move_id) {
...
void Move_List_Qml::cut_off(const int move_id)
{
    ...
    if (it != branches.end()) {
        ...
        if (vParent->childItems().length() == 2) {
            ...
            auto branch_start_id = QQmlProperty(buttons_to_delete->childItems().at(2), "move_id").read().toInt();
